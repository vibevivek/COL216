...
int main(){
    ...
    while(i<n){
        ...
        if(in==0 || in==1 || in==2 || in==5){
            ...
            if (arg[0]==symbol || arg[1]==symbol || arg[2]==symbol){
                checker_flag=1;
                if (span>cycle_count){
                    cycle_count++;
            ...
            cycle_count++;
            selector.push_back({m,checker_flag,cycle_count,span});
    ...
    for (int i=0; i<selector.size(); i++){
        ...
        if(cmd == 0){
            ...
            if(dst!=-1){
                ...
                TOTAL_CYCLE++;
                cout<<"cycle "<<selector[i][2]<<": "<<rev_register_map[dst]<<"="<<val3<<endl;
            }
            ...
            if(dst!=0 && register_file(p1) < register_file(p2)){
                register_File[dst] = 1;
                TOTAL_CYCLE++;
                cout<<"cycle "<<selector[i][2]<<": "<<rev_register_map[dst]<<"="<<1<<endl;
            }
            else if(dst!=0){
                register_File[dst] = 0;
                TOTAL_CYCLE++;
                cout<<"cycle "<<selector[i][2]<<": "<<rev_register_map[dst]<<"="<<0<<endl;
            }
            ...
            if (selector[i][1]==0){
                ...
                if(r!=0){
                    ...
                    TOTAL_CYCLE++;
                    cout<<"cycle "<<selector[i][2]<<": DRAM request issued"<<endl;
                }
            }
            else{
                instr_exec_cnt[7]++;
                ...
                if(r!=0){
                    ...
                    if (flag==false){
                        ...
                        TOTAL_CYCLE++;
                        cout<<"cycle "<<selector[i][2]<<"-"<<selector[i][3]<<": "<<rev_register_map[r]<<"= "<<value<<endl;
                        update++;
                        flag=true;
                    }
                    else{
             ...
             if (selector[i][1]==0){
                ...
                if(r!=0){
                    ...
                    if (flag==false){
                        ...
                        col = ((val-INSTR_MEM)/8)%1024;
                        TOTAL_CYCLE++;
                        cout<<"cycle "<<selector[i][2]<<"-"<<selector[i][3]<<": memory address "<<(val-INSTR_MEM)/8<<"-"<<((val-INSTR_MEM)/8)+3<<"= "<<register_file(r)<<endl;
                        update++;
                        flag=true;
                    }
                    else{
